...
export default function DashboardSidebar({ open, setOpen }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 transform border-r border-gray-200 dark:border-white/10 bg-white dark:bg-slate-900 p-5 transition-transform duration-300 md:static md:translate-x-0 md:flex md:flex-col ${
              ...
              <Link
                ...
                className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition-all duration-200 ${
                  active
                    ...
                    : 'text-gray-700 dark:text-gray-200 hover:bg-amber-50 hover:text-amber-600 dark:hover:bg-white/5 dark:hover:text-amber-300'
                }`}
              >
                <Icon size={20} />
                {item.name}
